...
int tlb_contains(unsigned x) {  // TODO: FINISHED

  for(int i = 0; i < 16; i++){
      if (x == tlb[i][0]){ return i; }
    }

  return -1;
}

void update_tlb(unsigned page) {  // TODO: FINISHED
...
unsigned getframe(FILE* fstore, unsigned logic_add, unsigned page,
         int *page_fault_count, int *tlb_hit_count) {              // TODO : FINISHED
...
int get_available_frame(unsigned page) {    // TODO : FINISHED
